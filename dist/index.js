#!/usr/bin/env node
import { createRequire as __secretsCreateRequire } from 'node:module'; const require = __secretsCreateRequire(import.meta.url);
...
function requireJsonSize(value, name, maximum) {
  let encoded;
  try {
    encoded = JSON.stringify(value);
  } catch (error) {
    throw new ModelReviewError(`Model review ${name} must be JSON-serializable: ${error instanceof Error ? error.message : String(error)}`, { code: "invalid_model_request" });
  }
  if (encoded === void 0) {
...
// node_modules/@adversarylabs/sdk/dist/index.js
var DEFAULT_INPUT_PATH = "/adversary/input.json";
var DEFAULT_OUTPUT_PATH = "/adversary/output.json";
var ADVERSARY_RUN_PROTOCOL_VERSION = 1;
var verboseValues = /* @__PURE__ */ new Set(["1", "true", "TRUE", "yes", "YES"]);
var envelopeValidator;
var Severity = {
  Info: "info",
  Low: "low",
  Medium: "medium",
...
var RuleRegistry = class _RuleRegistry {
  ...
  lookup(ruleId) {
    ...
    return rule === void 0 ? void 0 : cloneRuleDefinition(rule);
  }
  has(ruleId) {
    return this.rules.has(ruleId);
  }
  snapshot() {
    const snapshot = new _RuleRegistry();
...
function cloneRuleDefinition(rule) {
  return {
    ...rule,
    groupBy: rule.groupBy === void 0 ? void 0 : [...rule.groupBy]
  };
}
function cloneReviewPolicy(policy) {
  return {
    ...policy,
    confidenceThresholds: policy.confidenceThresholds === void 0 ? void 0 : { ...policy.confidenceThresholds },
    severityOverrides: policy.severityOverrides === void 0 ? void 0 : { ...policy.severityOverrides }
  };
}
...
var Adversary = class {
  ...
  async run(options) {
    ...
    const output = buildReviewResult({
      ...
      repository: repoPath,
      filesScanned: typeof summary.files_scanned === "number" ? summary.files_scanned : void 0,
      collector,
  ...
  async runFromEnvironment(options = {}) {
    const input = options.input ?? await parseInput(options.inputPath ?? process.env.ADVERSARY_INPUT ?? DEFAULT_INPUT_PATH);
...
function createAdversaryRunEnvelope(result) {
  return {
...
function toWireReviewResult(result) {
  return omitUndefined({
    ...
    assessment: result.assessment === void 0 ? void 0 : omitUndefined({ ...result.assessment }),
    ...
    opinion: result.opinion === void 0 ? void 0 : omitUndefined({ ...result.opinion }),
    suppressed: result.suppressed,
    timing: result.timing === void 0 ? void 0 : omitUndefined(result.timing),
...
function toWireFinding(finding) {
  return omitUndefined({
    ...
    remediation: finding.remediation === void 0 ? void 0 : omitUndefined({ ...finding.remediation }),
    tags: finding.tags,
...
function globPatternToRegExp(pattern) {
  const source = toPosixPath(pattern).replace(/[.+^${}()|[\]\\]/g, "\\$&").replace(/\*\*/g, "\0").replace(/\*/g, "[^/]*").replace(/\?/g, "[^/]").replace(/\0/g, ".*");
...
function buildReviewResult(input) {
  ...
  for (const finding of ranked) {
    const suppressed = confidenceWeight(finding.confidence) < confidenceWeight(minimumConfidence) || !includeInformational && finding.severity === Severity.Info || eligible.length >= maximumFindings;
    if (suppressed) {
  ...
  return omitUndefined({
    ...
    timing: input.timing,
    suppressedFindings: input.includeSuppressed ? suppressedFindings : void 0,
    rawObservations: input.includeRawObservations ? input.collector.observations : void 0
  });
}
function synthesizeObservationFindings(observations, thresholds, registry) {
  ...
  const findings = [...grouped.entries()].map(([groupKey, group]) => {
    ...
    const synthesisSource = rule?.aggregate === void 0 ? "generic" : "rule";
    ...
    const recommendation = synthesis.recommendation === void 0 ? synthesizeRecommendation(group) : normalizeParagraph(synthesis.recommendation);
    ...
    const finding = {
      ...
      title: synthesis.title ?? synthesizeObservationTitle(first.title, group.length, first.groupedTitle),
      category: synthesis.category ?? first.category ?? rule?.category ?? "general",
      severity: synthesis.severity ?? severity,
      confidence: synthesis.confidence ?? (rule?.defaultConfidence === void 0 ? confidence : normalizeConfidence(rule.defaultConfidence, thresholds)),
      ...
      evidence,
      recommendation: recommendation.length > 0 ? recommendation : void 0,
...
function normalizeFindingInput(input, occurrence = 0) {
  return omitUndefined({
    id: input.id ?? stableId(`${input.ruleId ?? input.title}:${input.groupKey ?? input.category}${input.deduplicate === false ? `:${occurrence}` : ""}`),
    ruleId: input.ruleId,
    ...
    recommendation: input.recommendation === void 0 ? void 0 : normalizeParagraph(input.recommendation),
    remediation: input.remediation,
    tags: input.tags === void 0 ? void 0 : uniqueStrings(input.tags),
...
function normalizeEvidence(input) {
  const legacy = input;
  const hasLocation = legacy.location !== void 0 || legacy.file !== void 0 || legacy.line !== void 0 || legacy.endLine !== void 0;
  const location = hasLocation ? omitUndefined({
...
function normalizeReviewNote(note) {
  return omitUndefined({
    ...note,
    evidence: note.evidence === void 0 ? void 0 : deduplicateEvidence(note.evidence)
  });
}
...
function synthesizeAssessment(findings, positives = []) {
  ...
  const primaryConcern = primary === void 0 ? "the highest-ranked finding" : assessmentConcern(primary);
...
function synthesizeObservationTitle(title, count, groupedTitle) {
  if (typeof title === "string") {
    return count > 1 && groupedTitle !== void 0 ? groupedTitle : title;
  }
...
function summarizeObservationGroup(group) {
  ...
  const recommendation = typeof first.recommendation === "string" ? void 0 : first.recommendation?.summary;
  if (group.length === 1) {
...
function synthesizeRecommendation(group) {
  const recommendations = uniqueStrings(group.flatMap((observation) => {
    ...
    if (observation.recommendation !== void 0) {
      return [
...
function defaultObservationGroupKey(observation, rule) {
  ...
  if (groupBy !== void 0 && groupBy.length > 0) {
    return groupBy.map((field) => `${field}:${stringFromUnknown(observationValue(observation, field)) ?? ""}`).join("|");
  }
...
function formatObservationLocation(observation) {
  if (observation.location?.file === void 0) {
    return void 0;
  }
  if (observation.location.line === void 0) {
...
function normalizeSemanticText(value) {
  return value.toLowerCase().replace(/[^a-z0-9]+/g, " ").split(/\s+/).filter((word) => word.length > 0 && !semanticStopWords.has(word)).join(" ");
}
...
function calibrateFindingSeverity(finding, policy) {
  const override = policy.severityOverrides?.[finding.ruleId ?? ""] ?? policy.severityOverrides?.[finding.groupKey ?? ""];
  return override === void 0 ? finding : { ...finding, severity: override };
}
function scoreFinding(finding) {
  ...
  return severityWeight(finding.severity) * 10 + confidenceWeight(finding.confidence) * 12 + locationScore + remediationScore;
}
...
function aggregateConfidence(values, strategy) {
  ...
  if (strategy === "minimum") {
    return [...values].sort((left, right) => confidenceWeight(left) - confidenceWeight(right))[0] ?? Confidence.Low;
  }
  if (strategy === "average") {
    const average = values.reduce((total, confidence) => total + confidenceNumericValue(confidence), 0) / values.length;
    return normalizeConfidence(average);
  }
  return [...values].sort((left, right) => confidenceWeight(right) - confidenceWeight(left))[0] ?? Confidence.Low;
}
...
function assertObservationInit(value, source, registry) {
  ...
  if (value.category === void 0 && rule?.category === void 0) {
...
function assertReviewScore(value) {
  ...
  if (value.max !== void 0 && (typeof value.max !== "number" || !Number.isFinite(value.max))) {
  ...
  if (value.max !== void 0 && (value.max <= 0 || value.score > value.max)) {
...
function assertReviewPolicy(policy, source) {
  ...
  if (policy.confidenceThresholds !== void 0) {
...
function optionalPositiveInteger(value, field) {
  if (value !== void 0 && (!Number.isInteger(value) || Number(value) < 1)) {
...
function isSeverity(value) {
  return value === Severity.Info || value === Severity.Low || value === Severity.Medium || value === Severity.High || value === Severity.Critical;
}
...
function formatScore(score) {
  ...
  const summary = score.summary === void 0 ? "" : ` - ${normalizeParagraph(score.summary)}`;
...
// src/rules.ts
var byId = new Map(spec.rules.map((rule) => [rule.id, rule]));
function registerRules(app) {
...
function observationFor(detection) {
  const rule = byId.get(detection.rule.id);
  if (rule === void 0) throw new Error(`Unknown rule ${detection.rule.id}`);
  return {
    ruleId: rule.id,
...
// src/model-review.ts
var MAX_MODEL_FILES = 12;
var MAX_FILE_CHARS = 5e3;
var MAX_DETERMINISTIC = 40;
var MAX_OBS = 6;
var SECRETS_MODEL_PROMPT = `You are reviewing a repository for committed secrets and credential material.
...
function prepareFactoryModelInput(detections, sources, changeFiles = []) {
...
` : source.content;
  ...
  return {
    ...
    request: {
      ...
      budget: { maximumOutputTokens: 4096, timeoutMs: 12e4 }
    }
  };
}
async function runModelSecretsReview(ctx, detections, sources, staticSeverities = [], staticPrimaryConcern) {
...
async function applyModelSecretsReview(ctx, output, evidenceById, staticSeverities, staticPrimaryConcern) {
  const rank = (s) => ({ none: 0, low: 1, medium: 2, high: 3, critical: 4 })[s] ?? 0;
  const modelSevs = output.observations.map((o) => o.severity);
  const all = ["none", output.assessment.risk, ...staticSeverities, ...modelSevs];
  const risk = all.reduce((best, cur) => rank(cur) > rank(best) ? cur : best, "none");
  ...
  for (const observation of output.observations.slice(0, MAX_OBS)) {
    const evidence = observation.evidenceIds.map((id) => evidenceById.get(id)).filter((item) => item !== void 0).slice(0, 8).map((item) => ({
      location: { file: item.path, ...item.line === void 0 ? {} : { line: item.line } },
...
function evaluate(rule, sources, allPaths) {
  ...
  if (match.kind === "query-credential-http-error") {
    return sources.filter((file) => match.files.some((glob) => matchesGlob(file.path, glob))).flatMap((file) => findQueryCredentialHttpErrors(rule, file));
  }
...
function hasRequestsReceiverProof(fileSource, block, receiver, requestIndex) {
...
function matchesGlob(path, glob) {
  ...
  for (let index = 0; index < glob.length; index += 1) {
    ...
    else pattern += character !== void 0 && "^$+?.()|{}[]".includes(character) ? "\\" + character : character;
  }
...
if (process.argv[1] !== void 0 && import.meta.url === new URL(process.argv[1], "file:").href) {
